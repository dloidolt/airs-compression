				 ...
				 uint32_t src_size, uint32_t src_samples)
{
	...
	if (work_len)
		work = arena_alloc(a, 1, (ptrdiff_t)work_len, sizeof(uint16_t));
	...
	e->dst = arena_alloc(a, (ptrdiff_t)e->dst_cap, 1, CMP_DST_ALIGNMENT);
...
void test_detect_too_small_work_buffer_in_model_preprocessing(const struct t_fixture *fix)
{
	...
	void *dst = arena_alloc(g_a, (ptrdiff_t)dst_cap, 1, CMP_DST_ALIGNMENT);
